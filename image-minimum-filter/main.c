...
void minimumFilter(unsigned char* _inputImgData, unsigned char* _outputImgData,
...
int main() {
  ...
  const char newImgName[] = "lena_min.bmp";

  imageReader(imgName, &imgHeight, &imgWidth, &imgBitDepth, &imgHeader[0],
              &imgColorTable[0], &imgBuffer[0]);

  minimumFilter(&imgBuffer[0], &imgBuffer2[0], imgWidth, imgHeight);
...
void minimumFilter(unsigned char* _inputImgData, unsigned char* _outputImgData,
  ...
  n = 5;
  for (y = n / 2; y <= imgRows; y++) {
    for (x = n / 2; x <= imgCols; x++) {
      smin = 255;
      for (j = -n / 2; j <= n / 2; j++) {
        for (i = -n / 2; i <= n / 2; i++) {
          a[i + n / 2][j + n / 2] =
